...
ListBox::ListBox(HWND hwnd) : m_x(0), m_y(0), m_BgColor(RGB(255, 255, 255)), m_ForeColor(RGB(0, 0, 0)), m_BgColorBrush(0), m_Font(0), m_OldFont(0)
{
	// Create the edit box
	m_hWndEdit = CreateWindow(TEXT("LISTBOX"), NULL, WS_BORDER | WS_CHILD | WS_TABSTOP | WS_VSCROLL | WS_HSCROLL, 0, 0, 0, 0, hwnd, NULL, NULL, NULL);
...
void ListBox::SetBounds(int x, int y, int width, int height)
{
	m_x = x;
	m_y = y;
...
void ListBox::SetEnabled(bool bEnable)
{
...
void ListBox::SetText(tstring const& textRef)
{
	SendMessage(m_hWndEdit, WM_SETTEXT, 0, (LPARAM) textRef.c_str());
}

void ListBox::SetFont(tstring const& fontNameRef, bool bold, bool italic, bool underline, int size)
{
	LOGFONT ft;
...
void ListBox::SetForecolor( COLORREF color )
{
	m_ForeColor = color;
...
void ListBox::SetBackcolor( COLORREF color )
{
	m_BgColor = color;
...
COLORREF ListBox::GetForecolor()
{
	return m_ForeColor;
}

COLORREF ListBox::GetBackcolor()
{
	return m_BgColor;
}

HBRUSH ListBox::GetBackcolorBrush()
{
	return m_BgColorBrush;
}

LRESULT ListBox::EditProcStatic(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	#pragma warning(disable: 4312)
	return reinterpret_cast<ListBox*>(GetWindowLong(hWnd, GWL_USERDATA))->EditProc(hWnd, msg, wParam, lParam);
	#pragma warning(default: 4312)
}

LRESULT ListBox::EditProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	switch (msg)
	{		
